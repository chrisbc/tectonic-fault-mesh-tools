...
valid_dip_directions = list(dip_direction_ranges.keys()) + [
    None,
    "SUBVERTICAL AND VARIABLE",
]

dominant_rake_ranges = {
    "reverse": (45, 135),
    "dextral": (135, 225),
    "sinistral": (315, 45),
    "normal": (225, 315),
}
secondary_rake_ranges = {
    "reverse": (0, 180),
    "dextral": (90, 270),
    "sinistral": (270, 90),
    "normal": (180, 360),
...
def reverse_line(line: LineString):
    ...
    if line.has_z:
        z_back = z[-1::-1]
        new_line = LineString(
            [[xi, yi, zi] for xi, yi, zi in zip(x_back, y_back, z_back)]
        )
    else:
...
class CfmMultiFault:
    ...
    def __init__(
        ...
        exclude_region_polygons: list = None,
        ...
        if exclude_region_polygons is not None:
            ...
            for i, row in fault_geodataframe.iterrows():
                if not any(
                    [row.geometry.within(poly) for poly in exclude_regions_nztm]
                ):
        ...
        if exclude_zero:
            trimmed_fault_gdf = trimmed_fault_gdf[trimmed_fault_gdf.SR_pref > 0.0]

        # Exclude upper slope faults (A-US classification)
        if exclude_aus:
            trimmed_fault_gdf = trimmed_fault_gdf[
                trimmed_fault_gdf.Fault_stat != "A-US"
            ]

        if depth_type != "D90":
            assert min_dfc >= 0.0
            trimmed_fault_gdf = trimmed_fault_gdf[
                trimmed_fault_gdf.Depth_Dfc >= min_dfc
            ]
    ...
    def add_fault(
        self, series: pd.Series, depth_type: str = "D90", remove_colons: bool = False
    ):
        cfmFault = CfmFault.from_series(
            series,
...
class CfmFault:
    def __init__(self, parent_multifault: CfmMultiFault = None):
        ...
        self._depth_best, self._depth_min, self._depth_max, self._depth_stdev = (
            None,
        ) * 4
        (
            self._dip_best,
    ...
    def depth_max(self, depth: Union[float, int]):
        ...
        for depth_value in (self.depth_min, self.depth_best):
            if depth_value is not None and depth_v < depth_value:
                print(
                    "Warning: depth_max lower than either depth_min or depth_best ({})".format(
                        self.name
                    )
                )
                self.logger.warning(
                    "depth_max lower than either depth_min or depth_best"
                )
    ...
    def depth_min(self, depth: Union[float, int]):
        ...
        for depth_value in (self.depth_max, self.depth_best):
            if depth_value is not None and depth_v > depth_value:
                print(
                    "Warning: depth_min higher than either depth_max or depth_best ({})".format(
                        self.name
                    )
                )
                self.logger.warning(
                    "depth_min higher than either depth_max or depth_best"
                )
    ...
    def dip_best(self, dip: Union[float, int]):
        ...
        if self.dip_min is not None:
            if dip_v < self.dip_min:
                print(
                    "{}: dip_best ({}) lower than dip_min ({})".format(
                        self.name, dip_v, self.dip_min
                    )
                )
        if self.dip_max is not None:
            if dip_v > self.dip_max:
                print(
                    "{}: dip_best ({}) greater than dip_max ({})".format(
                        self.name, dip_v, self.dip_max
                    )
                )
    ...
    def dip_max(self, dip: Union[float, int]):
        dip_v = self.validate_dip(dip)
        for key, dip_value in zip(
            ["dip_min", "dip_best"], [self.dip_min, self.dip_best]
        ):
            if dip_value is not None and bearing_leq(dip_v, dip_value):
                print(
                    "{}: dip_max ({}) is lower than {} ({})".format(
                        self.name, dip_v, key, dip_value
                    )
                )
    ...
    def dip_min(self, dip: Union[float, int]):
        dip_v = self.validate_dip(dip)
        for key, dip_value in zip(
            ["dip_max", "dip_best"], [self.dip_max, self.dip_best]
        ):
    ...
    def dip_sigma(self):
        if self._dip_sigma is not None:
            ...
            raise ValueError(
                "Insufficient data to calculate dip_sigma!"
            )  # <= not sure if you need this step as this will go through validate_dip
    ...
    def validate_dip_direction(self, tolerance: float = 10.0):
        """
    ...
    def down_dip_vector(self):
        ...
        if self.dip_dir is None:
            ...
            x, y = np.cos(np.radians(self.dip_best)) * np.array(
    ...
    def down_dip_polygon(self):
        ...
        if abs(self.down_dip_vector[-1]) > 1.0e-3:
            bottom_trace = (
                surface_trace_array
                + -1.0
                * self.down_dip_vector[:-1]
                * (self.depth_best / self.down_dip_vector[-1])
                * 1.0e3
            )
        else:
            bottom_trace = surface_trace_array + self.down_dip_vector[:-1] * 100.0e3

        combined_polygon_array = np.vstack((surface_trace_array, bottom_trace[::-1]))
        return Polygon(combined_polygon_array)

    def surface_trace_buffer(
        self,
        buffer_distance: Union[int, float] = 100.0,
        cap_style: int = 2,
        join_style: int = 2,
    ):
        """
        ...
        :return:
        """
        return self.nztm_trace.buffer(
            buffer_distance, cap_style=cap_style, join_style=join_style
        )

    def combined_buffer_polygon(
        self, buffer_distance, cap_style: int = 2, join_style: int = 2, wgs: bool = True
    ):
        trace_buffer = self.surface_trace_buffer(
            buffer_distance, cap_style=cap_style, join_style=join_style
        )
    ...
    def rake_best(self, rake: Union[float, int]):
        ...
        if self.rake_min is not None:
            if rake_v < self.rake_min:
                ...
                print(
                    "{}: rake_best ({}) lower than rake_min ({})".format(
                        self.name, rake_v, self.rake_min
                    )
                )
        ...
        if self.rake_max is not None:
            if rake_v > self.rake_max:
                ...
                print(
                    "{}: rake_best ({}) greater than rake_max ({})".format(
                        self.name, rake_v, self.rake_max
                    )
                )
    ...
    def rake_max(self, rake: Union[float, int]):
        rake_v = self.validate_rake(rake)
        for key, rake_value in zip(
            ["rake_min", "rake_best"], [self.rake_min, self.rake_best]
        ):
            if rake_value is not None and bearing_leq(rake_v, rake_value):
                print(
                    "{}: rake_max ({}) is lower than {} ({})".format(
                        self.name, rake_v, key, rake_value
                    )
                )
    ...
    def rake_min(self, rake: Union[float, int]):
        rake_v = self.validate_rake(rake)
        for key, rake_value in zip(
            ["rake_max", "rake_best"], [self.rake_max, self.rake_best]
        ):
            if rake_value is not None and bearing_geq(rake_v, rake_value):
                ...
                print(
                    "{}: rake_min ({}) is higher than {} ({})".format(
                        self.name, rake_v, key, rake_value
                    )
                )
    ...
    def validate_rake_sense(self):
        if any([a is None for a in (self.rake_best, self.sense_dom)]):
            print(
                "{}: Insufficient data to compare rake and slip sense".format(self.name)
            )
            ...
            if not all(
                [
                    bearing_geq(self.rake_best, dominant_range[0]),
                    bearing_leq(self.rake_best, dominant_range[1]),
                ]
            ):
                print(
                    "{}: Supplied rake ({:.2f} deg) differs from dominant slip sense ({})".format(
                        self.name, self.rake_best, self.sense_dom
                    )
                )
            ...
            if self.sense_sec is not None:
                ...
                if not all(
                    [
                        bearing_geq(self.rake_best, sec_range[0]),
                        bearing_leq(self.rake_best, sec_range[1]),
                    ]
                ):
                    print(
                        "{}: Supplied rake ({:.2f} deg) inconsistent with sec slip sense ({})".format(
                            self.name, self.rake_best, self.sense_sec
                        )
                    )
                    self.logger.warning(
                        "Supplied rake inconsistent with sec slip sense"
                    )
    ...
    def sr_best(self, slip_rate: Union[float, int]):
        ...
        if self.sr_min is not None:
            if slip_rate < self.sr_min:
                print(
                    "{}: sr_best ({.2f}) lower than sr_min ({.2f})".format(
                        self.name, slip_rate, self.sr_min
                    )
                )
        if self.sr_max is not None:
            if slip_rate > self.sr_max:
                print(
                    "{}: sr_best ({.2f}) greater than sr_max ({.2f})".format(
                        self.name, slip_rate, self.sr_max
                    )
                )
    ...
    def sr_min(self, slip_rate: Union[float, int]):
        ...
        for key, sr_value in zip(["sr_max", "sr_best"], [self.sr_max, self.sr_best]):
            if sr_value is not None and bearing_geq(slip_rate, sr_value):
                print(
                    "{}: sr_min ({:.2f}) is higher than {} ({:.2f})".format(
                        self.name, slip_rate, key, sr_value
                    )
                )
    ...
    def sr_max(self, slip_rate: Union[float, int]):
        ...
        for key, sr_value in zip(["sr_min", "sr_best"], [self.sr_max, self.sr_best]):
            if sr_value is not None and bearing_leq(slip_rate, sr_value):
                print(
                    "{}: sr_max ({:.2f}) is lower than {} ({:.2f})".format(
                        self.name, slip_rate, key, sr_value
                    )
                )
    ...
    def from_series(
        cls,
        series: pd.Series,
        parent_multifault: CfmMultiFault = None,
        depth_type: str = "D90",
        remove_colons: bool = False,
    ):
        ...
        fault.dip_best, fault.dip_min, fault.dip_max = (
            series["Dip_pref"],
            series["Dip_min"],
            series["Dip_max"],
        )
        ...
        fault.rake_best, fault.rake_min, fault.rake_max = (
            series["Rake_pref"],
            series["Rake_minus"],
            series["Rake_plus"],
        )
        fault.sense_dom, fault.sense_sec = series["Dom_sense"], series["Sub_sense"]
        fault.sr_best, fault.sr_min, fault.sr_max = (
            series["SR_pref"],
            series["SR_min"],
            series["SR_max"],
        )
    ...
    @classmethod
    def from_series_old(
        cls, series: pd.Series, parent_multifault: CfmMultiFault = None
    ):
        assert isinstance(series, pd.Series)
        fault = cls(parent_multifault=parent_multifault)
        ...
        fault.depth_best, fault.depth_min, fault.depth_max = (
            series["Depth_Best"],
            series["Depth_Min"],
            series["Depth_Max"],
        )
    ...
    def to_segment(self, x0: float, y0: float, x1: float, y1: float, num_columns: int):
        ...
        seg_str = (
            f"{self.name},{x0_deg:d},{x0_mins:.1f},{y0_deg:d},{y0_mins:.1f},"
    ...
    def trace_to_hybrid_csv(self, num_columns: int):
        ...
        out_str = self.pad_commas(
            f"{self.name},{len(coord_ls) - 1}D", num_columns=num_columns
        )
        out_str += self.to_segment(x[0], y[0], x[-1], y[-1], num_columns=num_columns)

        for c0, c1 in zip(coord_ls[:-1], coord_ls[1:]):
            out_str += self.to_segment(
                c0[0], c0[1], c1[0], c1[1], num_columns=num_columns
            )
    ...
    def fault_to_hybrid_csv(self, num_columns: int):
        data_str = (
            f"{self.name},{int(self.dip_min):d},{int(self.dip_best):d},{int(self.dip_max):d},"
