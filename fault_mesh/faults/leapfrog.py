...
from fault_mesh.faults.generic import (GenericFault, GenericMultiFault,
                                       calculate_strike_direction,
                                       normalize_bearing, smallest_difference)
from fault_mesh.faults.mesh import FaultMesh
from fault_mesh.utilities.cutting import (cut_line_at_point,
                                          cut_line_between_two_points)
...
from fault_mesh.utilities.opensha import fault_polygon_xml, fault_trace_xml
...
class LeapfrogMultiFault(GenericMultiFault):
    def __init__(
        ...
        sort_sr: bool = False,
        segment_distance_tolerance: float = 100.0,
        smoothing_n: int = None,
        ...
        super(LeapfrogMultiFault, self).__init__(
            fault_geodataframe=fault_geodataframe,
            sort_sr=sort_sr,
            remove_colons=remove_colons,
            dip_choice=dip_choice,
    ...
    def add_fault(
        ...
        cfmFault = LeapfrogFault.from_series(
            series,
            parent_multifault=self,
            remove_colons=remove_colons,
            tolerance=tolerance,
        ...
        if self.smoothing_n is None:
            ...
            cfmFault.smoothed_trace = smooth_trace(
                cfmFault.nztm_trace, n_refinements=self.smoothing_n
            )
    ...
    def curated_faults(self, fault_list: list):
        ...
        assert all(
            [
                isinstance(fault, (ConnectedFaultSystem, LeapfrogFault))
        ...
        for fault in self.faults:
            for other_fault in self.faults:
                if other_fault.name != fault.name:
                    if (
                        fault.nztm_trace.distance(other_fault.nztm_trace)
                        <= self.segment_distance_tolerance
                    ):
                        if verbose:
                        ...
                        for p1, p2 in product(
                            [fault.end1, fault.end2],
                            [other_fault.end1, other_fault.end2],
                        ):
                            conditions.append(
                                p1.distance(p2) <= self.segment_distance_tolerance
                            )
                        if any(conditions):
    ...
    def read_fault_systems(self, fault_system_csv: str, trimming_gradient: float = 1.0):
        ...
        with open(fault_system_csv, "r") as in_id:
            ...
            for line in con_data:
                ...
                if any([seg in self.names for seg in segs]):
                    print(
                        f"Creating connected fault system: {name} with segments: {[seg for seg in segs if seg]}"
                    )
                    cfault = ConnectedFaultSystem(
                        overall_name=name,
                        cfm_faults=self,
                        segment_names=segs,
    ...
    def from_nz_cfm_shp(
        cls,
        filename: str,
        exclude_region_polygons: List[Polygon] = None,
        depth_type: str = "D90",
        exclude_region_min_sr: float = 1.8,
        include_names: list = None,
        exclude_aus: bool = True,
        exclude_zero: bool = True,
        sort_sr: bool = False,
        remove_colons: bool = False,
        smoothing_n: Union[int, None] = 5,
        dip_choice: str = "pref",
        epsg: int = None,
        trimming_gradient: float = 1.0,
        dip_multiplier: float = 1.0,
    ...
    def from_shp(
        ...
        trimmed_fault_gdf = cls.gdf_from_nz_cfm_shp(
            filename=filename,
            exclude_region_polygons=None,
            depth_type=None,
            exclude_region_min_sr=1.8,
            include_names=None,
            exclude_aus=False,
            exclude_zero=False,
        )
    ...
    def find_inter_fault_connections(self):
        ...
        for con in self.connections:
            ...
            f1_name = (
                f1.name if not f1.is_segment else self.multi_segment_dict[f1.name].name
            )
            f2_name = (
                f2.name if not f2.is_segment else self.multi_segment_dict[f2.name].name
            )
            if f1_name != f2_name:
                connection_set.add(
                    tuple(sorted([f1_name, f2_name], key=lambda x: x.lower()))
                )
    ...
    def adjust_traces_for_terminations(
        self,
    ...
    def to_opensha_xml(
        self,
        exclude_subduction: bool = True,
        buffer_width: float = 5000.0,
        write_buffers: bool = True,
        subduction_names: tuple = ("hikurangi", "puysegur"),
    ):
        """
        ...
        for fault in self.faults:
            ...
            exclude_condition = all(
                [
                    exclude_subduction,
                    any([name in fault.name.lower() for name in subduction_names]),
                ]
            )
            # Add XML for fault
    ...
    def to_quads_mesh(
        self,
        sampled_dip: bool = True,
        depth_multiplier: float = 0.8,
        file_name: str = None,
    ):
        """
        ...
        meshes = []
        for fault in self.faults:
            vertices, quads = fault.to_quads(
                sampled=sampled_dip, depth_multiplier=depth_multiplier
            )
...
class LeapfrogFault(GenericFault):
    ...
    def __init__(
        self,
        parent_multifault: LeapfrogMultiFault = None,
        smoothing: int = 5,
        trimming_gradient: float = 1.0,
        segment_distance_tolerance: float = 100.0,
    ):
    ...
    def neighbouring_segments(self, segment_list: list):
        ...
        if len(segment_list) > 2:
            raise ValueError(
                f"Too many ({len(segment_list):d}) neighbours supplied for segment {self.name:s}.\n"
        ...
        for seg in segment_list:
            if seg.nztm_trace.distance(self.end1) <= self.segment_distance_tolerance:
                ...
                self._neighbour_angle_dict[self.end1.coords[0]] = self.neighbour_angle(
                    seg
                )
            elif seg.nztm_trace.distance(self.end2) <= self.segment_distance_tolerance:
                self._neighbour_dict[self.end2.coords[0]] = seg
                self._neighbour_angle_dict[self.end2.coords[0]] = self.neighbour_angle(
                    seg
                )
            else:
                raise ValueError(
                    f"Orphan neighbour segment for {self.name}: {seg.name}"
                )
    ...
    @property
    def strike(self):
        return normalize_bearing(self.dip_dir - 90.0)
    ...
    def across_strike_vector(self):
        ...
        return np.array([np.sin(dip_dir_rad), np.cos(dip_dir_rad), 0.0])
    ...
    def sampled_dip(self, dip: float):
        ...
        assert 0.0 <= dip <= 180.0, "Dip must be between 0 and 180 degrees"
    ...
    def generate_sampled_dip(self, from_parent=True):
        ...
        if from_parent and self.parent is not None:
            self.sampled_dip = (
                self.dip_min + (self.dip_max - self.dip_min) * self.parent.sampled_dip
            )
        else:
            self.sampled_dip = (
                self.dip_min + (self.dip_max - self.dip_min) * np.random.rand()
            )
    ...
    def down_dip_vector_sampled(self):
        ...
        if self.dip_dir is None:
            ...
            x, y = np.cos(np.radians(self.sampled_dip)) * np.array(
                [np.sin(np.radians(self.dip_dir)), np.cos(np.radians(self.dip_dir))]
            )
        return np.array([x, y, -z])

    def depth_contour(
        self, depth: float, smoothing: bool = True, km=False, sr_and_rake: bool = False
    ):
        """
        ...
        if self.is_segment:
            ...
            if len(self.neighbouring_segments) > 1:
                e1_box = self.end_clipping_box(
                    self.end1, depth, gradient_adjustment=self.trimming_gradient
                )
                e1_box_intersection = e1_box.boundary.intersection(self.nztm_trace)
                e2_box = self.end_clipping_box(
                    self.end2, depth, gradient_adjustment=self.trimming_gradient
                )
                e2_box_intersection = e2_box.boundary.intersection(self.nztm_trace)

                if not e1_box.intersects(e2_box):
                    trimmed_contour = cut_line_between_two_points(
                        shifted_contour, [e1_box_intersection, e2_box_intersection]
                    )
                else:
                ...
                if all([point.within(e1_box) for point in (contour_e1, contour_e2)]):
                    ...
                    if isinstance(e1_box_intersection, Point):
                        split_line = cut_line_at_point(
                            shifted_contour, e1_box_intersection
                        )

                        if split_line[0].distance(contour_e1) == 0.0:
                            trimmed_contour = split_line[1]
                        elif split_line[1].distance(contour_e1) == 0.0:
                ...
                if all([point.within(e2_box) for point in (contour_e1, contour_e2)]):
                    ...
                    if isinstance(e2_box_intersection, Point):
                        split_line = cut_line_at_point(
                            shifted_contour, e2_box_intersection
                        )

                        if split_line[0].distance(contour_e2) == 0.0:
                            trimmed_contour = split_line[1]
                        elif split_line[1].distance(contour_e2) == 0.0:
                            trimmed_contour = split_line[0]
                        else:
                            print("neither intersects")
                            trimmed_contour = None
    ...
    def generate_depth_contours(
        ...
        if trace:
            contours = [self.nztm_trace] + [
                self.depth_contour(depth, smoothing, km) for depth in depths
            ]
        else:
    ...
    def depth_contour_mesh(
        self,
        depths: Union[np.ndarray, List[float]],
        smoothing: bool = False,
        km: bool = False,
        trace: bool = False,
        point_spacing: float = 1000.0,
        ...
        contour_spline = spline_fit_contours(
            self.contours, point_spacing=point_spacing, output_spacing=output_spacing
        )
    ...
    def mesh_fault_surface(
        ...
        if self.contours is None:
            raise ValueError(
                "No contours to mesh. Please run generate_depth_contours() first."
            )

        contour_spline = spline_fit_contours(
            self.contours, point_spacing=spline_resolution, output_spacing=resolution
        )
    ...
    def nztm_trace(self, trace: LineString):
        ...
        if trace.has_z:
            new_trace = LineString([(xi, yi, 0.0) for xi, yi, _ in trace.coords])
        else:
            new_trace = LineString([(xi, yi, 0.0) for xi, yi in trace.coords])
    ...
    def clipping_box(
        self,
        centre_point: Point,
        along_half_width: float,
        across_half_width: float = 100000.0,
    ):
        ...
        out_array = point_array + np.array(
            [
    ...
    def end_clipping_box(
        self,
        end_i: Point,
        depth: float,
        gradient_adjustment: float = 1.0,
        across_half_width: float = 10000.0,
    ):
        end_angle = self.neighbour_angle_dict[end_i.coords[0]]
        end_width = (
            gradient_adjustment
            * np.tan(np.radians(end_angle))
            * (depth / np.sin(np.radians(self.dip_best)))
        )
    ...
    def neighbour_angle(self, neighbour: LeapfrogFault):
        ...
        if strike_diff > 90.0:
            strike_diff = 180.0 - strike_diff
            dip_diff = 180 - self.dip_best - neighbour.dip_best
            if dip_diff > 90.0:
                raise ValueError(
                    f"{self.name} and {neighbour.name}: shallow dips in opposite directions."
                    "Are you sure you want to connect?"
                )
        else:
            dip_diff = abs(neighbour.dip_best - self.dip_best)
        return max(
            [self.dip_multiplier * dip_diff, self.strike_multiplier * strike_diff]
        )
    ...
    def calculate_footprint(self, smoothed: bool = True, buffer: float = 5000.0):
        if smoothed:
    ...
    def adjust_footprint(self):
        ...
        if terms:
            ...
            for fault in cutting_faults:
                for nearest_end, other_end in zip(
    ...
    def extend_footprint(
        ...
        end_i: Point,
        other_end: Point,
        other_segment: LeapfrogFault,
        deepest_contour_depth: float = 30.0e3,
        search_line_length: float = 1.5e5,
        buffer_size: float = 5.0e3,
        fall_back_distance: float = 40.3,
    ):
        """
        ...
        if self.is_segment:
            ...
            while bottom_trace is None:
                ...
                contour_depth -= 2000.0
        else:
            bottom_trace = list(self.contours.geometry)[-1]
        bot1 = np.array(bottom_trace.coords)[0]
        bot2 = np.array(bottom_trace.coords)[-1]

        def distance_along_strike(
            point: np.array, reference: np.array, strike_vector: np.array
        ):
        ...
        bot_i = (
            bot1
            if (
                distance_along_strike(bot1, np.array(end_i.coords), strike_direction)
                > distance_along_strike(bot2, np.array(end_i.coords), strike_direction)
            )
            else bot2
        )

        search_line = LineString([bot_i, bot_i + search_line_length * strike_direction])
        ...
        if other_contour.intersects(bottom_trace):
            ...
            if isinstance(trace_intersection, MultiPoint):
                trace_intersection = min(
                    list(trace_intersection.geoms),
                    key=lambda x: x.distance(Point(bot_i)),
                )
            contour_intersection = search_line.intersection(other_contour)
            if isinstance(contour_intersection, MultiPoint):
                contour_intersection = min(
                    list(contour_intersection.geoms),
                    key=lambda x: x.distance(Point(bot_i)),
                )

            if any([a.is_empty for a in [trace_intersection, contour_intersection]]):
                if all(
                    [a.is_empty for a in [trace_intersection, contour_intersection]]
                ):
                ...
                corner_point = max(
                    [contour_intersection, trace_intersection],
                    key=lambda x: x.distance(Point(bot_i)),
                )

            triangle = Polygon(
                np.vstack(
                    [bot_i, np.array(corner_point.coords), np.array(end_i.coords)]
                )
            ).buffer(buffer_size, cap_style=2)

            if self.is_segment:
                new_boundary = unary_union(
                    [self.parent_connected_fault.footprint, triangle]
                )
    ...
    def extend_trace(
        ...
        local_strike_direction = calculate_strike_direction(
            coords_to_fit[:, 0], coords_to_fit[:, 1]
        )
        local_strike_vector = np.array(
            [
                np.sin(np.radians(local_strike_direction)),
                np.cos(np.radians(local_strike_direction)),
                0.0,
            ]
        )
    ...
    def to_xml(
        self, section_id: int, buffer_width: float = 5000.0, write_buffers: bool = True
    ):
        # Unique fault identifier
        tag_name = "i{:d}".format(section_id)
        # Metadata
        attribute_dic = {
            "sectionId": "{:d}".format(section_id),
        ...
        if write_buffers:
            ...
            polygon_element = fault_polygon_xml(
                self.combined_buffer_polygon(buffer_width), self.name
            )
    ...
    def to_quads(self, sampled: bool = True, depth_multiplier: float = 0.8) -> dict:
        ...
        if sampled:
            bottom_trace = (
                top_trace
                + self.down_dip_vector_sampled
                * depth_multiplier
                * self.depth_best
                * 1000.0
                * -1.0
                / self.down_dip_vector_sampled[-1]
            )
        else:
            bottom_trace = (
                top_trace
                + self.down_dip_vector
                * depth_multiplier
                * self.depth_best
                * 1000.0
                * -1.0
                / self.down_dip_vector[-1]
            )

        vertices = np.vstack([top_trace, bottom_trace])

        quad_list = []
        for top_i, bottom_i in zip(
            range(len(top_trace) - 1), range(len(bottom_trace) - 1)
        ):
            quad_list.append(
                [
                    top_i,
                    top_i + 1,
                    len(top_trace) + bottom_i + 1,
                    len(top_trace) + bottom_i,
                ]
            )
