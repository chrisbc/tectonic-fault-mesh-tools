...
def calculate_strike_direction(x: np.ndarray, y: np.ndarray):
    ...
    strike = normalize_bearing(bearing - 90.0)
    return strike
...
class GenericMultiFault:
    ...
    def add_fault(
        self, series: pd.Series, remove_colons: bool = False, tolerance: float = 100.0
    ):
        fault = GenericFault.from_series(
            series,
    ...
    def gdf_from_nz_cfm_shp(
        ...
        if depth_type is not None:
            if depth_type == "D90":
                ...
                trimmed_fault_gdf["Depth_std"] = 0.0
            ...
            trimmed_fault_gdf["Depth_pref"] = 3.0e4
            trimmed_fault_gdf["Depth_std"] = 0.0
...
class GenericFault:
    def __init__(
        self, parent_multifault: GenericMultiFault = None, tolerance: float = 100.0
    ):
        """
    ...
    def from_series(
        cls,
        series: pd.Series,
        parent_multifault: GenericMultiFault = None,
        remove_colons: bool = False,
