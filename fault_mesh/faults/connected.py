...
from fault_mesh.faults.generic import (calculate_dip_direction,
                                       dip_direction_ranges, reverse_bearing,
...
from fault_mesh.utilities.cutting import (cut_line_at_multiple_points,
                                          cut_line_at_point)
from fault_mesh.utilities.merging import (
    align_two_nearly_adjacent_segments,
    merge_multiple_nearly_adjacent_segments)
from fault_mesh.utilities.meshing import (fit_plane_to_points,
                                          get_strike_dip_from_normal,
                                          most_common_or_first,
...
class ConnectedFaultSystem:
    ...
    def __init__(
        ...
        excluded_names: Union[str, list] = None,
        tolerance: float = 100.0,
        smooth_trace_refinements: int = 5,
        trimming_gradient: float = 1.0,
    ):
        ...
        for fault in cfm_faults.faults:
            ...
            if search_patterns is not None:
                ...
                if any(
                    [fnmatch.fnmatch(name, pattern) for pattern in search_pattern_list]
                ):
                    # Ensure fault name is not in the exclusion list
                    if not any(
                        [fnmatch.fnmatch(name, pattern) for pattern in excluded_list]
                    ):
            ...
            if segment_names is not None:
                ...
                if any(
                    [fnmatch.fnmatch(name, pattern) for pattern in segment_names_list]
                ):
                    segments.append(fault)
        ...
        for segment in segments:
            ...
            other_segments = MultiLineString(
                [other_seg.nztm_trace for other_seg in segments if other_seg != segment]
            )
            ...
            if closest_dist > tolerance:
                ...
                for other_seg in segments:
                    if other_seg != segment:
                        if (
                            segment.nztm_trace.distance(other_seg.nztm_trace)
                            < tolerance
                        ):
        ...
        sorted_segments_s_to_n = sorted(
            segments, key=lambda x: (x.nztm_trace.bounds[0], x.nztm_trace.bounds[1])
        )
        sorted_segments_w_to_e = sorted(
            segments, key=lambda x: (x.nztm_trace.bounds[1], x.nztm_trace.bounds[0])
        )
        ...
        while len(ordered_segments) < len(segments) and max_iterations > 0:
            ...
            for segment in segments:
                if segment not in ordered_segments:
                    if (
                        segment.nztm_trace.distance(ordered_segments[-1].nztm_trace)
                        <= tolerance
                    ):
                        ordered_segments.append(segment)
        if max_iterations == 0:
            raise ValueError(
                f"Could not order segments for connected fault {overall_name}"
            )
        self.segments = ordered_segments
        if self._smooth_trace_refinements is None:
            overall_trace = merge_multiple_nearly_adjacent_segments(
                [seg.nztm_trace for seg in self.segments], densify=None
            )
        else:
            overall_trace = merge_multiple_nearly_adjacent_segments(
                [seg.nztm_trace for seg in self.segments]
            )
        ...
        for l1, l2 in zip(self.segments, self.segments[1:]):
            if l1.nztm_trace.distance(l2.nztm_trace) == 0.0:
                boundaries.append(l1.nztm_trace.intersection(l2.nztm_trace))
            else:
                if self._smooth_trace_refinements is None:
                    new_l1, new_l2 = align_two_nearly_adjacent_segments(
                        [l1.nztm_trace, l2.nztm_trace],
                        tolerance=tolerance,
                        densify=None,
                    )
                else:
                    new_l1, new_l2 = align_two_nearly_adjacent_segments(
                        [l1.nztm_trace, l2.nztm_trace], tolerance=tolerance
                    )
        ...
        if smooth_trace_refinements is not None:
            self.smoothed_overall_trace = smooth_trace(
                self.overall_trace, n_refinements=smooth_trace_refinements
            )
            if len(self.segment_boundaries) >= 2:
                self.smoothed_segments = cut_line_at_multiple_points(
                    self.smoothed_overall_trace, self.segment_boundaries
                )
            else:
                self.smoothed_segments = cut_line_at_point(
                    self.smoothed_overall_trace, self.segment_boundaries[0]
                )
            for smoothed_segment in self.smoothed_segments:
                closest_segment = min(
                    self.segments,
                    key=lambda x: x.nztm_trace.centroid.distance(
                        smoothed_segment.centroid
                    ),
                )
    ...
    def depth_contour(
        ...
        contours = [
            segment.depth_contour(depth, smoothing) for segment in self.segments
        ]
        valid_contours = [contour for contour in contours if contour is not None]
        return MultiLineString(valid_contours)

    def generate_depth_contours(
        self,
        depths: Union[np.ndarray, List[float]],
        smoothing: bool = True,
        damping: int = None,
        km: bool = False,
    ):
        ...
        if max(depths) > 0:
            depths *= -1
        if self.parent.epsg is not None:
            self.contours = gpd.GeoDataFrame(
                {"depth": depths}, geometry=contours, crs=self.parent.epsg
            )
        else:
            self.contours = gpd.GeoDataFrame({"depth": depths}, geometry=contours)

    def generate_simple_contours(
        self, depths: Union[np.ndarray, List[float]], km: bool = False
    ):
        ...
        if self.dip_dir is None:
            # Assume vertical
            dd_vec = np.array([0.0, 0.0, -1])
        else:
            z = np.sin(np.radians(average_dip))
            x, y = np.cos(np.radians(average_dip)) * np.array(
                [np.sin(np.radians(self.dip_dir)), np.cos(np.radians(self.dip_dir))]
            )
        ...
        if max(depths) > 0:
            depths *= -1
        if self.parent.epsg is not None:
            simple_contours = gpd.GeoDataFrame(
                {"depth": depths}, geometry=contours, crs=self.parent.epsg
            )
        else:
    ...
    def calculate_overall_dip_direction(self, tolerance: float = 10.0):
        """
        Compares dip direction string (e.g. NW) with
        :return:
        """
        dip_dir_strings = [
            seg.dip_dir_str for seg in self.segments if seg.dip_dir_str is not None
        ]
        ...
        if any([a is None for a in [dd_str, self.nztm_trace]]):
            ...
            if dd_str != "SUBVERTICAL AND VARIABLE":
                ...
                if dd_str != "N":
                    if not all(
                        [
                            min_dd_range - tolerance <= dd_from_trace,
                            dd_from_trace <= max_dd_range + tolerance,
                        ]
                    ):
                        reversed_dd = reverse_bearing(dd_from_trace)
                        if all(
                            [
                                min_dd_range - tolerance <= reversed_dd,
                                reversed_dd <= max_dd_range + tolerance,
                            ]
                        ):
                            ...
                            print(
                                "{}: Supplied trace and dip direction {} are inconsistent: expect either {:.1f}"
                                "or {:.1f} dip azimuth. Please check...".format(
                                    self.name,
                                    self.dip_dir_str,
                                    dd_from_trace,
                                    reversed_dd,
                    ...
                    if any(
                        [
                            315.0 - tolerance <= dds
                            for dds in [dd_from_trace, reversed_dd]
                        ]
                    ):
                        ...
                        print(
                            "{}: Supplied trace and dip direction {} are inconsistent: expect either {:.1f} or {:.1f}"
                            " dip azimuth. Please check...".format(
                                self.name, self.dip_dir_str, dd_from_trace, reversed_dd
                            )
                        )
                        self.logger.warning(
                            "Supplied trace and dip direction are inconsistent"
                        )

                        self._dip_dir = dd_from_trace

    def generate_sr_rake_points(
        self,
        depths: Union[np.ndarray, List[float]],
        smoothing: bool = True,
        km: bool = False,
        ...
        if self.parent.epsg is not None:
            sr_rake_points = gpd.GeoDataFrame(
                {"depth": point_depths, "slip_rate": point_srs, "rake": point_rakes},
                geometry=point_geoms,
                crs=self.parent.epsg,
            )
        else:
            sr_rake_points = gpd.GeoDataFrame(
                {"depth": point_depths, "slip_rate": point_srs, "rake": point_rakes},
    ...
    def overall_trace(self, trace: Union[LineString, MultiLineString]):
        if isinstance(trace, LineString):
            ...
            self._overall_trace = merge_multiple_nearly_adjacent_segments(
                list(trace.geoms), tolerance=self.segment_distance_tolerance
            )
    ...
    def trace_and_contours(self, smoothed: bool = True):
        ...
        if smoothed:
            return unary_union(
                [self.smoothed_trace]
                + list(self.contours.geometry)
                + list(self.end_lines(smoothed=smoothed).geoms)
            )
        else:
            return unary_union(
                [self.overall_trace]
    ...
    def end_polygon(self, smoothed: bool = False, distance: float = 1.0e5):
        if smoothed:
        ...
        if smoothed:
            assert end1.distance(seg1.smoothed_trace) < 500.0
            assert end2.distance(seg2.smoothed_trace) < 500.0
        else:
            assert end1.distance(seg1.nztm_trace) < 500.0
            assert end2.distance(seg2.nztm_trace) < 500.0
        ...
        if line1.intersects(line2):
            ...
            p1 = max(
                [p for p in line1.coords], key=lambda x: Point(x).distance(intersection)
            )
            p2 = max(
                [p for p in line2.coords], key=lambda x: Point(x).distance(intersection)
            )
            edge_poly = Polygon([p1, intersection, p2])

        else:
            edge_poly = Polygon(
                np.vstack([np.array(line1.coords), np.array(line2.coords)])
            )
    ...
    def calculate_footprint(self, buffer: float = 15000.0):
        if self.parent.smoothing_n is not None:
            footprint = (
                self.trace_and_contours(smoothed=True)
                .minimum_rotated_rectangle.buffer(buffer, cap_style=2)
                .intersection(self.end_polygon(smoothed=True))
            )
        else:
            footprint = (
                self.trace_and_contours(smoothed=False)
                .minimum_rotated_rectangle.buffer(buffer, cap_style=2)
                .intersection(self.end_polygon(smoothed=False))
            )
        self._footprint = footprint

    def end_lines(
        self, smoothed: bool = False, depth: float = 20.0e3, spacing: float = 2000.0
    ):
        if smoothed:
    ...
    def adjust_footprint(self):
        ...
        if terms:
            cutting_faults = [
                self.parent.curated_fault_dict[name]
                for name in terms
                if name is not self.name
            ]
            for fault in cutting_faults:
                for nearest_end, other_end in zip([end1, end2], [end2, end1]):
                    nearest_seg_this_fault = min(
                        self.segments, key=lambda x: x.nztm_trace.distance(nearest_end)
                    )
                    if nearest_end.distance(fault.nztm_trace) < 1.0e3:
                        if isinstance(fault, ConnectedFaultSystem):
                            closest_seg = min(
                                fault.segments,
                                key=lambda x: x.nztm_trace.distance(nearest_end),
                            )
                        else:
                            closest_seg = fault

                        nearest_seg_this_fault.extend_footprint(
                            nearest_end, other_end, closest_seg
                        )

    def adjust_trace(
        self,
        extend_distance: float = 20.0e3,
        fit_distance: float = 5.0e3,
        resolution: float = 1.0e3,
    ):
        if self.parent.smoothing_n is not None:
            end1 = Point(self.smoothed_overall_trace.coords[0])
            end2 = Point(self.smoothed_overall_trace.coords[-1])
            ...
            for fault in cutting_faults:
                for nearest_end, other_end in zip([end1, end2], [end2, end1]):
                    if nearest_end.distance(fault.nztm_trace) < 1.0e3:
                        ...
                        nearest_seg_this_fault.extend_trace(
                            nearest_end,
                            other_end,
        ...
        if self._smooth_trace_refinements is None:
            self.overall_trace = merge_multiple_nearly_adjacent_segments(
                [seg.nztm_trace for seg in self.segments], densify=None
            )
        else:
            self.overall_trace = merge_multiple_nearly_adjacent_segments(
    ...
    def mesh_fault_surface(
        self,
        resolution: float = 1000.0,
        spline_resolution: float = 100.0,
        plane_fitting_eps: float = 1.0e-5,
        check_mesh: bool = False,
        check_strike_dip: bool = False,
    ):
        ...
        :rtype: meshio.Mesh
        """
        spline_contours = self.generate_simple_contours(
            self.contours.depth.values, km=False
        )
        spline_contour_list = [
            np.array(contour.coords) for contour in spline_contours.geometry.values
        ]

        spline_contour_points = np.vstack(spline_contour_list)
        spline_contour_dict = {
            tuple(point): i for (i, point) in enumerate(spline_contour_points)
        }

        spline_contour_boundary = np.vstack(
            [
                spline_contour_list[0],
                np.vstack(
                    [
                        spline_contour_list[i][-1]
                        for i in range(1, len(spline_contour_list) - 1)
                    ]
                ),
                spline_contour_list[-1][::-1],
                np.vstack(
                    [
                        spline_contour_list[i][0]
                        for i in range(len(spline_contour_list) - 1, 0, -1)
                    ]
                ),
            ]
        )
        ...
        for contour in self.contours.geometry.values:
            if isinstance(contour, LineString):
                ...
                contours_list.append(
                    np.vstack([np.array(line.coords) for line in contour.geoms])
                )

        all_contour_points = np.vstack(contours_list)
        plane_normal, plane_origin = fit_plane_to_points(
            all_contour_points, eps=plane_fitting_eps
        )
        ...
        in_plane_x_vector = (
            in_plane_x_vector - np.dot(in_plane_x_vector, plane_normal) * plane_normal
        )
        ...
        in_plane_y_vector = np.array([0.0, 0.0, -1.0])
        ...
        for contour in contours_list:
            ...
            resolved_contours.append(
                np.vstack([rotated_contour_x, rotated_contour_y, rotated_contour_z]).T
            )
        ...
        for contour in spline_contour_list:
            ...
            # Create a new contour with the rotated coordinates
            resolved_spline_contours.append(
                np.vstack([rotated_contour_x, rotated_contour_y]).T
            )

            # refine end of spline contours by to match ends of fit contours

        refined_spline_contour_list = []
        for spline_contour, fit_contour in zip(
            resolved_spline_contours, resolved_contours
        ):
            ...
            if min_resolved_x_spline < min_resolved_x_fit:
                ...
                spline_contour = spline_contour[
                    spline_contour[:, 0] >= min_resolved_x_fit
                ]
            if max_resolved_x_spline > max_resolved_x_fit:
                spline_contour = spline_contour[
                    spline_contour[:, 0] <= max_resolved_x_fit
                ]
            ...
            if min_resolved_x_spline > min_resolved_x_fit:
                new_start = fit_contour[fit_contour[:, 0] <= min_resolved_x_spline][
                    :, :2
                ]
        ...
        for contour in refined_spline_contour_list:
            ...
            after_reprojection = (
                plane_origin
                + to_reproject[:, 0][:, np.newaxis]
                * np.repeat([in_plane_x_vector], to_reproject.shape[0], axis=0)
                + to_reproject[:, 1][:, np.newaxis]
                * np.repeat([in_plane_y_vector], to_reproject.shape[0], axis=0)
                + to_reproject[:, 2][:, np.newaxis]
                * np.repeat([plane_normal], to_reproject.shape[0], axis=0)
            )
            reprojected_contours.append(after_reprojection)
        reprojected_points = np.vstack(reprojected_contours)

        reprojected_contour_dict = {
            tuple(point): i for (i, point) in enumerate(reprojected_points)
        }

        reprojected_contour_boundary = np.vstack(
            [
                reprojected_contours[0],
                np.vstack(
                    [
                        reprojected_contours[i][-1]
                        for i in range(1, len(reprojected_contours) - 1)
                    ]
                ),
                reprojected_contours[-1][::-1],
                np.vstack(
                    [
                        reprojected_contours[i][0]
                        for i in range(len(reprojected_contours) - 1, 0, -1)
                    ]
                ),
            ]
        )
        ...
        interpolated_xyz = (
            plane_origin
            + all_resolved_spline_points[:, 0][:, np.newaxis]
            * np.repeat([in_plane_x_vector], interpolated.shape[0], axis=0)
            + all_resolved_spline_points[:, 1][:, np.newaxis]
            * np.repeat([in_plane_y_vector], interpolated.shape[0], axis=0)
            + interpolated[:, np.newaxis]
            * np.repeat([plane_normal], interpolated.shape[0], axis=0)
        )
        if check_mesh:
            plt.close("all")
            fig, ax = plt.subplots()
            ...
            ax.scatter(
                ...
                color="blue",
                s=1,
                label="Resolved boundary points",
            )
            plt.show(block=True)
        ...
        boundary_segments = np.array(
            [
                [
                    reprojected_contour_dict[tuple(reprojected_contour_boundary[i])],
                    reprojected_contour_dict[
                        tuple(reprojected_contour_boundary[i + 1])
                    ],
                ]
                for i in range(len(reprojected_contour_boundary) - 1)
            ]
        )
        boundary_segments = np.vstack(
            [
                boundary_segments,
                [
                    reprojected_contour_dict[tuple(reprojected_contour_boundary[-1])],
                    reprojected_contour_dict[tuple(reprojected_contour_boundary[0])],
                ],
            ]
        )
