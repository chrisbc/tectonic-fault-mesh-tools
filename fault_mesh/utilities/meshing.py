...
def most_common_or_first(string_list):
    ...
    tied_strings = [
        string for string, count in counter.items() if count == most_common_count
    ]
...
def triangulate_contours(
    ...
    contour_boundary = np.vstack(
        [
            contours_list[0],
            np.vstack([contours_list[i][-1] for i in range(1, len(contours_list) - 1)]),
            contours_list[-1][::-1],
            np.vstack(
                [contours_list[i][0] for i in range(len(contours_list) - 1, 0, -1)]
            ),
        ]
    )
    ...
    boundary_segments = np.array(
        [
            [
                all_contour_dict[tuple(contour_boundary[i])],
                all_contour_dict[tuple(contour_boundary[i + 1])],
            ]
            for i in range(len(contour_boundary) - 1)
        ]
    )
    boundary_segments = np.vstack(
        [
            boundary_segments,
            [
                all_contour_dict[tuple(contour_boundary[-1])],
                all_contour_dict[tuple(contour_boundary[0])],
            ],
        ]
    )
