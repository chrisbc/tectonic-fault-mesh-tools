...
def contour_mesh(
    mesh_name: str,
    depth_interval: float = 500.0,
    z_range: tuple = (-2.0e4, 0.0),
    espg: int = 2193,
) -> gpd.GeoSeries:
    """
    ...
    contours = mesh.contour(
        isosurfaces=int((z_range[1] - z_range[0]) / depth_interval) + 1,
        scalars="Z Height",
        rng=z_range,
    )
    ...
    contour_lines = gpd.GeoDataFrame(
        {"depth": contour_levels}, geometry=contour_lines, crs=espg
    )
