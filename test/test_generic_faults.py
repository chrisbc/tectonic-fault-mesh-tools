...
class test_generic_faults(TestCase):
    ...
    def test_add_fault(self):
        self.assertGreater(
            len(self.sorted_df), 0, "Not enough rows in the input data set"
        )
    ...
    def test_fault_numbers(self):
        ...
        self.assertFalse(
            len(self.fault_model.fault_numbers) == 0, "The fault number is missing"
        )
    ...
    def test_from_shp(self):
        ...
        self.assertTrue(
            response,
            'supplied object is not a GenericMultiFault"'
...
class test_generic_fault(TestCase):
    ...
    def test_dip_max(self):
        ...
        with self.assertLogs(logger=self.logger, level="WARNING") as cm:
            ...
            self.assertIn(
                "WARNING:fault_model_logger:dip_max is lower than dip min or dip best",
                cm.output,
            )
    ...
    def test_dip_min(self):
        ...
        with self.assertLogs(logger=self.logger, level="WARNING") as cm:
            ...
            self.assertIn(
                "WARNING:fault_model_logger:dip_min is higher than dip max or dip best",
                cm.output,
            )
    ...
    def test_dip_dir_str(self):
        ...
        with self.assertLogs(logger=self.logger, level="WARNING") as cm:
            ...
            self.assertIn(
                "WARNING:fault_model_logger:Supplied trace and dip direction are inconsistent",
                cm.output,
            )
    ...
    def test_validate_dip_direction(self):
        ...
        with self.assertLogs(logger=self.logger, level="WARNING") as cm:
            self.cmf_fault.validate_dip_direction()
            self.assertIn(
                "WARNING:fault_model_logger:Insufficient information to validate dip direction",
                cm.output,
            )
